<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
	<%@page import="java.sql.*" %>
	<%@page import="java.io.IOException" %>

<!DOCTYPE html>
<html>
<head>
	<meta charset="ISO-8859-1">
	<title>Ticket History | MindCare </title>
	<link rel = "stylesheet" href = "ticketHistory.css">
	<link rel = "stylesheet" href="header&footer.css">
</head>
<body>

	<!--header-->
    <header>
        <div class="navbar">
            <div class="logo">
                <img src="images/logo2.jpeg" alt="Logo" width="250px" height="100px">  
            </div>
            <ul class="links">
                <li class="nav"><a class="nav_a" href="home.jsp"><b>Home</b></a></li>
                <li class="nav">
                    <a class="nav_a" href="#"><b>Discover</b></a>
                    <ul class="submenu">
                        <li><a href="Doctor.jsp">Doctors</a></li>
                        <li><a href="disease.jsp">Diseases</a></li>
                    </ul>
                    
                </li>
                
                <li class="nav">
                    <a class="nav_a" href=""><b>Get help </b></a>
                    <ul class="submenu">
                        <li><a href="#">Appointment scheduling</a></li>
                        <li><a href="ticketSubmit.jsp">Ticket</a></li>
                        <li><a href="FeedbackInsert.jsp">Feedback</a></li>
                    </ul>
                </li>
                <li class="nav"><a class="nav_a" href="aboutUs.jsp"><b>About Us</b></a></li>
            </ul>
            <div class="shortcut">
                <a href="" class="btn_type1">Login</a>

                <div class="profile-img">
                    <a href="Profile.jsp" ><img src="images/profile.png" width="30px" height="30px" ></a>
                </div>
            </div>

        </div>
    </header>
    <!--end-->
        
        <br><br><br><br>


	<div class="name">
		<h2>Ticket history</h2>
	</div>
	

	<table class="table" >
		<thead>
			<tr>
				<th scope="col">Ticket no</th> 
				<th class = "seen" scope="col">Ticket Status</th>
				<th class = "seen" scope="col">Submitted Date </th>
				<th class = "seen" scope="col">Title</th>
				<th class = "seen" scope="col">Description</th>
				<th class = "seen" scope="col">Update</th>
				<th class = "seen" scope="col">Delete</th>
			</tr>
		</thead>
			
		<tbody>
		
			<%
			Connection con = null;
			Statement stmt = null;
			ResultSet rs = null;
			
			String url = "jdbc:mysql://localhost:3306/mindcareticket";
			String user = "root";
			String password = "200184";
			
			try{
				Class.forName("com.mysql.jdbc.Driver");
				
				con = DriverManager.getConnection(url, user, password); 
				stmt = con.createStatement();
				rs = stmt.executeQuery("select * from ticket");
				
				while(rs.next()){
			%>		
				
				<tr align = center>
					<th scope="row"> <% out.println(rs.getInt(1)); %> </th>
					<td class = "seen" scope="row"> <% out.println(rs.getString(2)); %> </td>
					<td class = "seen" scope="row"> <% out.println(rs.getString(3)); %> </td>
					<td class = "seen" scope="row"> <% out.println(rs.getString(4)); %> </td>
					<td class = "seen" scope="row"> <% out.println(rs.getString(5)); %> </td>
					<td>
						<a href = 'ticketUpdate.jsp?u=<% out.println(rs.getString("idticket")); %>'  class = button>Edit Ticket</a>
					</td>
					<td>
						<form action='delete?d=<%= rs.getString("idticket")%>' method="post">
							<input class ="button" type="submit" name="delete" value="Delete Ticket" id="<%= rs.getString("idticket") %>" >
						</form>
					</td>
				</tr>	
					
			<%	
				}
				
			}
			catch (Exception e){
				
			} %>
		</tbody>
	</table>
	
	<br><br><br><br>
	
	<!--Footer-->
    <footer>
        <div class="footer_info">
            <div class="footer_width about">
                <h2>About</h2>
                <p>
                    MindCare is a web application that provides informations about psychology and mainly focus on customer care services regarding to user needs.
                </p>
                <div class="social_media">
                    <ul>
                        <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                        <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                        <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                    </ul>
                </div>
            </div>

            <div class="footer_width link">
                <h2>Quick Links</h2>
                <ul>
                    <li><a href="home.jsp">Home</a></li>
                    <li><a href="#">FAQ</a></li>
                    <li><a href="PrivasyPolisy.jsp">Privacy Policy</a></li>
                    <li><a href="aboutUs.jsp">About Us</a></li>
                </ul>
            </div>

            <div class="footer_width contact">
                <h2>Contact</h2>
                <ul>
                    <li>
                        <span><img src="data:image/png;base64,"/></span>
                        <p>
                            26985 Brighton Lane, Lake Forest, CA 92630
                        </p>
                    </li>

                    <li>
                        <span><img src="data:image/png;base64,"/></span>
                        <a href="#">John@sajadhesami.com</a>
                    </li>

                    <li>
                        <span><img src="data:image/png;base64,"/></span>
                        <a href="#">+30-2106019311</a>
                    </li>
                </ul>
            </div>
        </div>
        <hr>
        <div class="copy-right">
            <p>
                Copyright and copyMindCare 2023 All Rights Reserved.
            </p>
        </div>
    </footer>
<!--end-->

</body>
</html>